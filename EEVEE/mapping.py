...
class Maze(object):
    ...
    def update(self, my_x, my_y, left_sensor, front_sensor, right_sensor, back_sensor,ir_left_sensor, ir_right_sensor,
        ...
        if dist_to_cell_center < 45/4: # [1/4 cm da cell] : o robo tem 20cm e a celula 45cm
            self.my_cell.explored = True
        ...
        dist_sensor_from_robot_center = 0
        dist_ir_from_robot_center = 8  #[cm]
        # 0º
        ...
        self.update_single_sensor(left_sensor, nearby_cells,
                                  [left45_sensor_wall_pos],
                                  left45_sensor_pos_in_eevee)
        ...
        self.update_single_sensor(ir_left_sensor, nearby_cells,
                                  [left_sensor_wall_pos],
        ...
        right45_sensor_pos_in_eevee = [self.eevee[0] + dist_sensor_from_robot_center * math.cos(compass + math.pi / 4),
                                     self.eevee[1] + dist_sensor_from_robot_center * math.sin(compass + math.pi / 4)]
        right45_sensor_wall_pos = [right45_sensor_pos_in_eevee[0] + min_right_sensor / 2 * math.cos(compass + math.pi / 4),
        ...
        self.update_single_sensor(right_sensor, nearby_cells,
                                  ...
                                  right45_sensor_pos_in_eevee)
        # 90º --> IR
        right_sensor_pos_in_eevee = [self.eevee[0] + dist_ir_from_robot_center * math.cos(compass + math.pi / 2),
                                     self.eevee[1] + dist_ir_from_robot_center * math.sin(compass + math.pi / 2)]
        right_sensor_wall_pos = [right_sensor_pos_in_eevee[0] + min_ir_right_sensor / 2 * math.cos(compass + math.pi / 2),
                                 right_sensor_pos_in_eevee[1] + min_ir_right_sensor / 2 * math.sin(compass + math.pi / 2)]

        self.update_single_sensor(ir_right_sensor, nearby_cells,
            ...
            for cell in nearby_cells:
                for wall in cell.walls:
                    ...
                    if d < ray_dists:
                        ray_dists = d
                        ray_walls = wall
            min_index = 0 #np.argmin(ray_dists)
