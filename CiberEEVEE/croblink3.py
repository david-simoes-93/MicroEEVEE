...
class CRobLink:
    def __init__(self, robName, robId, host):
        ...
        self.sock.sendto(msg.encode(), (host, UDP_PORT))
    ...
    def driveMotors(self, lPow, rPow):
        ...
        self.sock.sendto(msg.encode(), (UDP_IP, self.port))
    ...
    def startElement(self, name, attrs):
        if name == "Reply":
            ...
            if id < NUM_IR_SENSORS:
                ...
                self.measures.irSensor[id] = float(attrs["Value"].replace(",", "."))
            else:
                self.status = -1
            ...
            if 1:
                ...
                if attrs["Value"] == "NotVisible":
                    ...
                    self.measures.beacon = (True, float(attrs["Value"].replace(",", ".")))
            else:
                self.status = -1
        elif name == "GPS":
            if "X" in attrs.keys():
                ...
                if "Dir" in attrs.keys():
                    ...
                    self.measures.dir = float(attrs["Dir"].replace(",", "."))
                else:
