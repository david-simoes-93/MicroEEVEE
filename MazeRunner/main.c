...
int main(void) {
    ...
    lastBeh=-1;
    int number_of_seconds_to_wait=180;

    //long my_time = readCoreTimer();

    while (davidcounter < number_of_seconds_to_wait*100) { //150*100
        //int prevAngle=0;
        printf("Press start to continue\n");
        while (!startButton());
        davidcounter = 0;
        //my_time = readCoreTimer();
        enableObstSens();
       ...
       //led(3,1);
        ...
        } while (!stopButton() && davidcounter < number_of_seconds_to_wait*100);
        disableObstSens();
        setVel2(0,0);
