...
/**
 * parses a XML message of Sensors type and gathers information in values
 * property
 */
        ...
        if (qName.equals("Measures")) {         // Measures Values
            ...
            if (attrs != null) {
                ...
                if (compStr != null) {
                    ...
                    try {
                        values.compass = nf.parse(compStr).doubleValue();
            ...
            if (attrs != null) {
                ...
                if (idStr != null) {
                    ...
                    if (IRValStr != null) {
                        ...
                        try {
                            values.IRSensor[IRid] = nf.parse(IRValStr).doubleValue();
                        } catch (ParseException ex) {
                            Logger.getLogger(SensorHandler.class.getName()).log(Level.SEVERE, null, ex);
                        }
                    }
                ...
                if (idStr != null) {
                    ...
                    if (valStr != null) {
                        ...
                        if (!valStr.equals("NotVisible")) {
                            try {
                                values.beacon[id].beaconDir = nf.parse(valStr).doubleValue();
                            } catch (ParseException ex) {
                                Logger.getLogger(SensorHandler.class.getName()).log(Level.SEVERE, null, ex);
                            }
            ...
            if (attrs != null) {
                ...
                if (xStr != null) {
                    try {
                        values.gpsData.x = nf.parse(xStr).doubleValue();
                    } catch (ParseException ex) {
                        Logger.getLogger(SensorHandler.class.getName()).log(Level.SEVERE, null, ex);
                    }
                ...
                if (yStr != null) {
                    try {
                        values.gpsData.y = nf.parse(yStr).doubleValue();
                    } catch (ParseException ex) {
                        Logger.getLogger(SensorHandler.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
                ...
                if (dirStr != null) {
                    try {
                        values.gpsData.dir = nf.parse(dirStr).doubleValue();
                    } catch (ParseException ex) {
                        Logger.getLogger(SensorHandler.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
...
/**
 * class ReplyHandler parses a XML message of Reply type and gathers information
 * in params property
 */
class ReplyHandler extends DefaultHandler {

    /**
     * includes values of simulation parameters it is not constructed internally
     */
        ...
        if (qName.equals("Parameters")) {     // Parameters Values
            ...
            if (attrs != null) {
                ...
                if (compassNoiseStr != null) {
                    try {
                        params.compassNoise = nf.parse(compassNoiseStr).doubleValue();
                    } catch (ParseException ex) {
                ...
                if (beaconNoiseStr != null) {
                    try {
                        params.beaconNoise = nf.parse(beaconNoiseStr).doubleValue();
                    } catch (ParseException ex) {
                        Logger.getLogger(ReplyHandler.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
                ...
                if (obstacleNoiseStr != null) {
                    try {
                        params.obstacleNoise = nf.parse(obstacleNoiseStr).doubleValue();
                    } catch (ParseException ex) {
                        Logger.getLogger(ReplyHandler.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
                ...
                if (motorsNoiseStr != null) {
                    try {
                        params.motorsNoise = nf.parse(motorsNoiseStr).doubleValue();
                    } catch (ParseException ex) {
                        Logger.getLogger(ReplyHandler.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
...
/**
 * class xmlParser creates the saxParser and parses xmlMessages of types Sensors
 * and Reply
 */
class xmlParser {
...
/**
 * provides the interface with the simulator, all types of communication with
 * the simulator may be realized using methods of this class
 */
public class ciberIF {

    /**
     * ciberIF contructor The constructor inits all necessary data structures,
     */
    public ciberIF() {
    ...
    /**
     * Initializes Robot and Connects to Simulator, allows specification of
     * obstacle sensors position
     *
     * @param name robot name
     * @param pos initial position of robot in initial grid
     * @param IRSensorAngles array of doubles that specifies position of IR
     * sensors, positions are given from angle (in degrees) from front of Robot
     * @param hostname host where simulator running
     */
    ...
    /**
     * Read a message from simulator. If no message is available wait, when
     * message arrives parses it. This is the main synchronization method of
     * this library.
     */
    ...
    /**
     * Indicates if a new Obstacle measure from sensor id has arrived.\ The
     * value of GetObstacleSensor is invalid when IsObstacleReady returns false
     */
    ...
    /**
     * get obstacle sensor measure, value is inversely proportional to nearest
     * obstacle distance
     */
    ...
    /**
     * get beacon sensor measure, value is the direction of beacon in robot
    ...
    /**
     * only when IsCompassReady returns true is the compass measure
     * significative
     */
    ...
    /**
     * get compass sensor measure, value is the direction of robot in ground
     * coordinates (-180.0,180.0)
     */
    ...
    public boolean NewMessageFrom(int id) {
        if (values.hearMessage[id - 1] == null) {
            return false;
        }
    ...
    /**
     * only when IsGPSReady returns true are the GPS measures (X, Y, Dir)
     * significative
     */
    ...
    /**
     * get x coordinate of current position from GPS sensor, can be used for
     * debug, invoke simulator with '-gps' option
     */
    public double GetX() {
    ...
    /**
     * get y coordinate of current position from GPS sensor, can be used for
     * debug, invoke simulator with '-gps' option
     */
    public double GetY() {
    ...
    /**
     * get robot orientation of current position from GPS sensor, can be used
     * for debug, invoke simulator with '-gps' option
     */
    ...
    /**
     * get returning led state, usefull to verify if SetReturningLed() was
    ...
    /**
     * get end led state, usefull to verify if Finished() was executed in the
     * simulator
     */
    ...
    /**
     * drive left motor with lPow and right motor with rPow, powers are in
     * (-0.15, 0.15)
     */
    public void DriveMotors(double lPow, double rPow) {

        send(new String("<Actions LeftMotor=\"" + lPow + "\" RightMotor=\"" + rPow + "\"/>"));
        setMotorsVal(lPow, rPow);
            //atStart();
        //myCompass(lPow, rPow);		//calculates the compass

    }

    /**
     * broadcast message to all other robots, the maximum size is limited
     */
