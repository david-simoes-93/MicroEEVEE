...
void ftw_execute() {
    ...
    if ((mainSide < 10 ||
        ...
        sharpTurnCounter = -1;
                                ...
                                25) {                        // MEDIUM WALL ON MAINSIDE (between 15 and 35) and far is_wall on weak side
        int dist = (mainSide - 25);                                // Dist between -10 and 10
        opposSpeed = 60 + dist;                                    // 70,50
        wallSpeed = 60 - dist;
        prevSpeed = (mainSide - 25) *
                    2;                                                    // prevSpeed between -20 and 20; positive means its away from is_wall
        sharpTurnCounter = -1;
    } else if (mainSide < 35 && weakSide <=
                                25) {                        // MEDIUM WALL ON MAINSIDE (between 15 and 35) but medium is_wall on weak side as well
...
bool ftw_isPossible() {
    ...
    if (!facedBeacon && visible && (wallOnTheRight || wallOnTheLeft) && sharpTurnCounter <= 0) {
        ...
        // if following is_wall, beacon has been visible all along, beacon is in front, no obstacles nearby and not in any loops
             ...
             sharpTurnCounter <= 0) { //&& grausRodados < 180 && grausRodados > -180) {
        wallOnTheRight = false;
        wallOnTheLeft = false;
    }
