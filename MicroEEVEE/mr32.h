/**
 * \file mr32.h
 * \brief MR32 library (Include File)
 *
...
/** \brief wait until tick80ms is set
 */
#define waitTick80ms() {while(!tick80ms); tick80ms = 0;}
/** \brief wait until tick40ms is set
 */
#define waitTick40ms() {while(!tick40ms); tick40ms = 0;}
/** \brief wait until tick20ms is set
 */
#define waitTick20ms() {while(!tick20ms); tick20ms = 0;}
/** \brief wait until tick10ms is set
 */
#define waitTick10ms() {while(!tick10ms); tick10ms = 0;}

/** \brief get start button status 
 * \return the start button status 
 */
#define startButton() (!PORTBbits.RB3) //*! get start button status
/** \brief get stop button status 
 * \return the stop button status 
 */
#define stopButton() (!PORTBbits.RB4) //*! get stop button status

/** \brief enable obstacle sensors
 */
#define enableObstSens() {LATBbits.LATB10=1;}
/** \brief disable obstacle sensors
 */
...
typedef union
{
	struct 
	{
		...
		int obstSensLeft;    ///< left obstacle sensor
		int an6;
		int an7;
		int batteryVoltage;  ///< battery voltage
	};
	int array[6];
} MR32_analogSensors;
 ...
 * \param value 1-bit value representing the required on/off state of the led
 */
