...
void get_next_point_in_astar_path();
    ...
    while (!stopButton()) {
        ...
        if (recalculate) {
            ...
            if (astar(rows, cols, sx, sy, gx, gy, paths)) {
                printf("Astar search from (%d,%d) to (%d,%d)\n", sx, sy, gx, gy);
...
void get_next_point_in_path() {
    int target_path_point_index = path_length - 1;
    if (path_length > 1)
        target_path_point_index = path_length - 2;

    get_gps_coords_from_cell_index(path_list[target_path_point_index][0], path_list[target_path_point_index][1],
