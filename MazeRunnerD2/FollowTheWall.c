...
void ftw_execute() {
    ...
    if ((mainSide < 10 ||
         midSide < 5)) {                                // REALLY CLOSE WALL ON MAINSIDE OR FRONT (less than 10)
        opposSpeed = -30;
        wallSpeed = 30;
    } else if (weakSide < 10 && mainSide >
                                20) {                        // REALLY CLOSE WALL ON WEAKSIDE; MEDIUM WALL ON MAINSIDE (less than 10)
        opposSpeed = 30;
        wallSpeed = -30;
        ...
        wallSpeed = 60 + dist / 2;
        sharpTurnCounter = -1;
    } else if (mainSide < 35 && weakSide >
                                25) {                        // MEDIUM WALL ON MAINSIDE (between 15 and 35) and far wall on weak side
        int dist = (mainSide - 25);                                // Dist between -10 and 10
        opposSpeed = 60 + dist;                                    // 70,50
        wallSpeed = 60 - dist;
        prevSpeed = (mainSide - 25) *
                    2;                                                    // prevSpeed between -20 and 20; positive means its away from wall
        sharpTurnCounter = -1;
    } else if (mainSide < 35 && weakSide <=
                                25) {                        // MEDIUM WALL ON MAINSIDE (between 15 and 35) but medium wall on weak side as well
...
bool ftw_isPossible() {
    ...
    else if (facedBeacon && visible && (wallOnTheRight || wallOnTheLeft) &&
             (lastKnownBeaconAngle < 60 && lastKnownBeaconAngle > -60)
             && obstValFront > 15 && mainSide > distanceNeededToAbandonWall &&
             sharpTurnCounter <= 0) {
        wallOnTheRight = false;
        wallOnTheLeft = false;
    }*/ if (facedBeacon && visible &&
            ((wallOnTheRight && visible && lastKnownBeaconAngle <= 0 && lastKnownBeaconAngle >= -60) ||
               (wallOnTheLeft && visible && lastKnownBeaconAngle >= 0 && lastKnownBeaconAngle <= 60))) {
        wallOnTheRight = false;
        wallOnTheLeft = false;
    }
